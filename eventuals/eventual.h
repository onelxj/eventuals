...
struct _Eventual {
  ...
  struct Adaptor final {
    ...
    void Fail(Error&& error) {
      ...
      static_assert(
          std::disjunction_v<
              std::is_same<std::exception_ptr, std::decay_t<Error>>,
              types_contains<std::exception, Errors_...>,
              types_contains<std::decay_t<Error>, Errors_...>>,
  ...
  struct Continuation final {
    ...
    void Fail(Args&&... args) {
      if constexpr (IsUndefined<Fail_>::value) {
        ...
        fail_(adaptor(), std::forward<Args>(args)...);
      } else {
        fail_(context_, adaptor(), std::forward<Args>(args)...);
      }
    }
  ...
  struct Builder final {
    ...
    using ErrorsFrom = tuple_types_union_t<std::tuple<Errors_...>, Errors>;
    ...
    auto raises() && {
      static_assert(
          std::tuple_size_v<std::tuple<Errors_...>> == 0,
          "Duplicate 'raises<...>'");

      return create<Interruptible_, Value_, Error, Errors...>(
          std::move(context_),
          std::move(start_),
          std::move(fail_),
          std::move(stop_));
